...
func instanceButtonsSoundSignals():
	...
	for inst in buttons:
		if !inst.pressed.is_connected(on_button_pressed):
...
func switch_level_by_id(level_id : int):
	if level_id == 1:
		GameState.currentLevel = 'ChangeDeck'
		backgroundMusicNode.stream = backgroundMusicBase
	elif level_id == 2:
		GameState.currentLevel = 'Friends'
		backgroundMusicNode.stream = backgroundMusicBase
	elif level_id == 3:
		GameState.currentLevel = 'Arena'
		backgroundMusicNode.stream = backgroundMusicMap
	elif level_id == 4:
		GameState.currentLevel = 'Ranking'
		backgroundMusicNode.stream = backgroundMusicBase
	elif level_id == 5:
		GameState.currentLevel = 'AddFriend'
		backgroundMusicNode.stream = backgroundMusicBase
	else:
		level_id = 0
		GameState.currentLevel = 'Home'
